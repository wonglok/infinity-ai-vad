...
export function useInit() {
  ...
  const runInit = useCallback(async () => {
    try {
      ...
      if (!supported) {
        setFatalError(
          "Browser not supported. Please use Chrome 96+ or Edge 96+.",
        );
        return;
      }
          ...
          try {
            ...
            await downloadModel(sdkId, (progress) => {
              ...
              if (progress > lastProgress) {
                ...
                console.warn(
                  `Download stalled for ${modelId}, still waiting...`,
                );
              }
            });
      ...
      if (cachedCount > 0) {
        console.log(
          `Using ${cachedCount}/${ALL_MODELS.length} models from OPFS cache`,
        );
      }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (attempt === 3) {
              ...
              setFatalError(
                `Failed to load model: ${modelId}. ${(err as Error).message}`,
              );
              return;
            }
